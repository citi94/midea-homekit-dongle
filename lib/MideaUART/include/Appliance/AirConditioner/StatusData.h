...
class StatusData : public FrameData {
 public:
  ...
  /* LOCAL PATCH (see PATCHES.md): fields present in the 0xC0 body that
     upstream does not expose. Offsets cross-checked against msmart-ng
     (StateResponse) and node-mideahvac (parsers/C0.js). */

  /// Fault code, byte 16. 0 = no fault.
  uint8_t getErrorCode() const { return this->m_getValue(16); }
  /// Appliance-in-error flag, byte 1 bit 7.
  bool getErrorFlag() const { return this->m_getValue(1, 128); }
  /// Filter / dust-full alert, byte 13 bit 5.
  bool getFilterAlert() const { return this->m_getValue(13, 32); }
  /// Panel display state: byte 14 bits 4-6, level 7 = off.
  bool getDisplayOn() const { return this->m_getValue(14, 0x70, 4) != 7; }
  /// Auxiliary electric (PTC) heat active, byte 9 bit 3.
  bool getAuxHeat() const { return this->m_getValue(9, 8); }
  /// Anion / purifier active, byte 9 bit 5.
  bool getPurifier() const { return this->m_getValue(9, 32); }
