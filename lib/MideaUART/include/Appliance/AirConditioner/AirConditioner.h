...
class AirConditioner : public ApplianceBase {
 public:
  ...
  /* LOCAL PATCH (see PATCHES.md): extra 0xC0 fields tracked from status */
  /// ms since the last successful status response (not just the last CHANGE)
  uint32_t getStatusAgeMs() const {
    return this->m_statusMs ? millis() - this->m_statusMs : UINT32_MAX;
  }
  uint8_t getErrorCode() const { return this->m_errorCode; }
  bool getFilterAlert() const { return this->m_filterAlert; }
  bool getDisplayOn() const { return this->m_displayOn; }
  bool getAuxHeat() const { return this->m_auxHeat; }
 protected:
