...
ResponseStatus AirConditioner::m_readStatus(FrameData data) {
  if (!data.hasStatus())
    return ResponseStatus::RESPONSE_WRONG;
  this->m_statusMs = millis();  /* LOCAL PATCH: liveness stamp */
  ...
  /* LOCAL PATCH: fault + panel state (see PATCHES.md) */
  setProperty(this->m_errorCode, newStatus.getErrorCode(), hasUpdate);
  setProperty(this->m_filterAlert, newStatus.getFilterAlert(), hasUpdate);
  setProperty(this->m_displayOn, newStatus.getDisplayOn(), hasUpdate);
  setProperty(this->m_auxHeat, newStatus.getAuxHeat(), hasUpdate);
  if (hasUpdate)
    this->sendUpdate();
  return ResponseStatus::RESPONSE_OK;
}
